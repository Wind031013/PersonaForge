...
class Config:
    ...
    # 新增：固定字符分隔的长度配置（例如每500个字符切一块）
    CHUNK_SIZE = 500
    # 新增：块之间的重叠长度（例如重叠50个字符，防止语义被生硬切断）
    CHUNK_OVERLAP = 50
...
class RagConstruction:
    ...
    def __init__(self, split_mode: str = "file", batch_size: int = 32):
        """
        :param split_mode: 分隔模式，可选 "file"(按原文件), "fixed"(固定字符), "paragraph"(按段落)
        :param batch_size: 批量向量化大小
        """
    ...
    def save_embeddings(self):
        ...
        try:
            ...
            # 提取 metadata 用于存入 ChromaDB (检索时可按文件名过滤)
            metadatas = [item["metadata"] for item in self.data]
            
            # 生成唯一的 ID (建议格式：文件名_切片索引，防止冲突)
...
if __name__ == "__main__":
    ...
    # 1. 测试固定字符切片 (每500字，重叠50字)
    print("--- 测试固定字符切片 ---")
    rag_fixed = RagConstruction(split_mode="fixed", batch_size=8)
    rag_fixed.data = rag_fixed.load_data()
    rag_fixed.save_embeddings()

    # 2. 测试段落切片 (按换行符和连续换行切分，去缩进)
    print("\n--- 测试段落切片 ---")
    rag_para = RagConstruction(split_mode="paragraph", batch_size=8)
    rag_para.data = rag_para.load_data()
    rag_para.save_embeddings()
    
    # 3. 测试原逻辑切片 (一整个文件作为一个chunk)
    # print("\n--- 测试文件切片 ---")
